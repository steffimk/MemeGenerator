...
class Gallery extends React.Component {
    ...
    renderImage(image, currentRoute) {
        ...
        return (
          <div className="image-container" id={image._id}>
            <Link to={imageRoute} key={image._id} onClick={() => viewMeme(image._id, Date.now(), this.isNotAuthenticated)}>
    ...
    likeImage = (id) => {
        ...
        authorizedFetch(endpointWithParam, 'GET', {}, this.isNotAuthenticated).then((docs) => {
            if(!docs.data.likes || !(docs.data.likes.map((like) => like.username)).includes(username)) {
        ...
        let newImages = this.state.images.map(img => {
            if(img._id === id) {
                if (img.likes && img.likes.map((like) => like.username).includes(username)) {
