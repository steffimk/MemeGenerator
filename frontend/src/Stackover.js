...
export default class Stackover extends React.Component {
    constructor(props) {
        super(props);
...
//   }
  if(myGif) { // If gif object defined
    if(!myGif.loading){  // if loaded
        ...
        ctx.drawImage(myGif.image,1,0);
        ctx.fillText("Halllo",60,60);
        ...
        ctx.fillText("WASSSSSSS geht hier den ab" + myGif.frames.length,50,60);
...
const GIF = function () {
    ...
    function loadGif(filename, isUrl) { // starts the load
       if(isUrl) {
    ...
    render(){
        return(
            <div>
            <h1>TEst</h1>
            <canvas ref={this.canvasRef} style={{width: 500}} />
            <img id="gif" src={testGif}></img>
